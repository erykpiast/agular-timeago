...
.filter('timeago', function(){
	...
	// returns full label
	function _getNumber(value, word, feminine, accusativ, half, oneAndHalf, ignoreOne) {
		var integer = Math.floor(value);
		var rest = value - integer;
	...
	function  _getLabel(value, unit, accusativ, half) {
		var	pDict = dict[unit]; // period dictionary
		var integer = Math.floor(value);
		var lastDigit = integer - Math.floor(value, -1);
		var secondForm = ((integer < 10 || integer > 20) && (lastDigit > 1 && lastDigit < 5));
		var label = '';
	...
	function _getValue(timestamp, half) {
		var value = {
				number: 1,
				unit: 'second'
			};
		...
		periods.forEach(function(period, index, periods) {
			if(timestamp >= period.milliseconds) {
				var val = timestamp / period.milliseconds;
				var rest = val - Math.floor(val);

				value.unit = period.name;

				if ((rest !== 0)
					&& (rest !== 0.5 || !period.half)) {
					var nextPeriod = periods[index + 1];

					if(nextPeriod) {
						var tVal = timestamp / nextPeriod.milliseconds;
